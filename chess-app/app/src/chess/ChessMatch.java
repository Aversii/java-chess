...
public class ChessMatch {
    ...
    public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition tgtPosition) {
        ...
        //Special Move Promotion needs to be verified before testCheck.
        promoted = null;
        if(movedPiece instanceof Pawn){
            if((movedPiece.getColor() == Color.WHITE && tgt.getRow() == 0) || (movedPiece.getColor() == Color.BLACK && tgt.getRow() == 7)){
        ...
        if(movedPiece instanceof Pawn && (tgt.getRow() == source.getRow()-2) ||(tgt.getRow() == source.getRow()+2)){
    ...
    private void validateSourcePosition(Position position) {
        ...
        if (currentPlayer != ((ChessPiece)board.piece(position)).getColor()){
            throw new ChessException("Its not your turn to play");
        }  
    ...
    private Piece makeMove(Position source, Position tgt) {
        ...
        //castling king side
        if(p instanceof King && tgt.getColumn() == source.getColumn()+2){
            Position srcR = new Position(source.getRow(), source.getColumn()+3);
            Position tgtR = new Position(source.getRow(), source.getColumn()+1);
            ChessPiece rook = (ChessPiece)board.removePiece(srcR);
            board.placePiece(rook, tgtR);
    ...
    private void undoMove(Position source, Position tgt, Piece capturedPiece){
        ChessPiece auxiliarPiece = (ChessPiece)board.removePiece(tgt);
        auxiliarPiece.decreaseMoveCount();
        board.placePiece(auxiliarPiece, source);
        ...
        if(auxiliarPiece instanceof King && tgt.getColumn() == source.getColumn()+2){
            Position srcR = new Position(source.getRow(), source.getColumn()+3);
            Position tgtR = new Position(source.getRow(), source.getColumn()+1);
            ChessPiece rook = (ChessPiece)board.removePiece(tgtR);
        ...
        if (auxiliarPiece instanceof Pawn){
            if(source.getColumn() != tgt.getColumn() && capturedPiece == enPassantVunerable){
                ...
                Position pawnPosition;
                if(auxiliarPiece.getColor()==Color.WHITE){
